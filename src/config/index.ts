...
const config = {
  ...
  // Videos per worker for the UNCLAIMED queue. 1 = one worker per queued video
  // (offset by idle/booting workers), up to MAX_WORKERS, so uploads transcode in
  ...
  // CFR TARGET frame rate for a near-static source. A slide deck / screen-hold can
  // carry an effective rate well under 1 fps, which ffmpeg passes straight through:
  // the HLS output then holds ~1 video frame per segment with avg_frame_rate=0/0,
  // which hls.js/MSE cannot play (fatal MEDIA_ERROR "Playback failed"). Only such
  // near-static sources (trigger floor in encoding/ffmpeg.ts, well below any real
  // rate so NORMAL videos are untouched) are lifted to this constant rate via a
  // per-output `-r`, duplicating the held frame into a playable timeline.
